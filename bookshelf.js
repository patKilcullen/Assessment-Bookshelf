...
class Bookshelf {
    ...
    searchTitle(word) {
      // this.bookArray.filter((book) => book.title === title);
      return this.bookArray.filter((book) =>
      book.title.find((string) => string.includes(word))
    );
    }
    searchAuthor(name) {
      return this.bookArray.filter((book) =>
 ...
 //NEEDOMETHING BAOUT EVEN?ODDD 
        const booksPerShelf = Math.floor(this.bookArray.length / this.numberOfShelves)
